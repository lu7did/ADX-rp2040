...
void USBAudio::init(EndpointResolver& resolver)
{
...
void USBAudio::callback_state_change(DeviceState new_state)
{
    ...
    if (_connected && (new_state != USBDevice::Configured)) {
        _receive_change(Closed);
        _send_change(Closed);
    }
}

uint32_t USBAudio::callback_request(const USBDevice::setup_packet_t *setup, USBDevice::RequestResult *result, uint8_t** data)
{
    assert_locked();
    ...
    *result = USBDevice::PassThrough;
    ...
    if ((setup->wIndex & 0xff) < pluggedInterface || (setup->wIndex & 0xff) >= pluggedInterface + numInterfaces) {
        return size;
    }
...
bool USBAudio::callback_request_xfer_done(const setup_packet_t *setup, bool aborted)
{
    assert_locked();
...
bool USBAudio::callback_set_configuration(uint8_t configuration)
{
    assert_locked();
    ...
    if (configuration == DEFAULT_CONFIGURATION) {
        ...
        PluggableUSBD().endpoint_add(_episo_out, _rx_packet_size_max, USB_EP_TYPE_ISO,  ::mbed::callback(this, &USBAudio::_receive_isr));
        PluggableUSBD().endpoint_add(_episo_in, _tx_packet_size_max, USB_EP_TYPE_ISO,  ::mbed::callback(this, &USBAudio::_send_isr));

        // activate readings on this endpoint
        PluggableUSBD().read_start(_episo_out, _rx_packet_buf, _rx_packet_size_max);
        ret = true;
    }
    return ret;
}
...
void USBAudio::_send_isr_next_sync()
{
    ...
    // Start the write
    PluggableUSBD().write_start(_episo_in, _tx_packet_buf, send_size);
    _tx_idle = false;
