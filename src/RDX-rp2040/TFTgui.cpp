...
ButtonWidget btnAGC=ButtonWidget(&tft);
ButtonWidget btnMODE=ButtonWidget(&tft);
...
class displayPDX {        // The class
  ...
  void setAGC(uint16_t _agc) {
    if (_agc==AGC_REF) {strcpy(btnPDX[BUTTON_AGC].label,"REF");}
    if (_agc==AGC_DECAY) {strcpy(btnPDX[BUTTON_AGC].label,"DECY");}
    if (_agc==AGC_ATTACK_FAST) {strcpy(btnPDX[BUTTON_AGC].label,"FAST");}
    if (_agc==AGC_ATTACK_SLOW) {strcpy(btnPDX[BUTTON_AGC].label,"SLOW");}
    if (_agc==AGC_GAIN_STEP) {strcpy(btnPDX[BUTTON_AGC].label,"GAIN");}
    if (_agc==AGC_OFF) {strcpy(btnPDX[BUTTON_AGC].label,"OFF");}
    showBtn(BUTTON_AGC);
  }
  void setAGC(uint16_t _agc,bool _inverse,bool _fickle) {
    btnPDX[BUTTON_AGC].inverse = _inverse;
    btnPDX[BUTTON_AGC].fickle   = _fickle;
    btnPDX[BUTTON_AGC].tfickle  = millis();
    setAGC(_agc);
  }
  ...
  void setMode(uint16_t _mode) {
    switch(_mode) {
      case 0: strcpy(btnPDX[BUTTON_MODE].label,"WSPR");break;
      case 1: strcpy(btnPDX[BUTTON_MODE].label,"JS8");break;
      case 2: strcpy(btnPDX[BUTTON_MODE].label,"FT4");break;
      case 3: strcpy(btnPDX[BUTTON_MODE].label,"FT8");break;
      case 4: strcpy(btnPDX[BUTTON_MODE].label,"CW");break;
      default : strcpy(btnPDX[BUTTON_MODE].label,"FT8");break;
    }
    showBtn(BUTTON_MODE);
  }

  void setMode(uint16_t _mode,bool _inverse,bool _fickle) {
    btnPDX[BUTTON_MODE].inverse=_inverse;
    btnPDX[BUTTON_MODE].fickle=_fickle;
    btnPDX[BUTTON_MODE].tfickle=millis();
    setMode(_mode);   
  ...
  void setTX(bool _tx) {
    if (_tx==true) {
       sprintf(hi,"%s","TX");
       setBtn(BUTTON_TX,hi,true,false);
       showBtn(BUTTON_TX);
    } else {
       sprintf(hi,"%s","RX");
       setBtn(BUTTON_TX,hi,false,false);
       showBtn(BUTTON_TX);
    }
  }
  void setTX(bool _tx,bool _inverse, bool _fickle) {
    if (_tx==true) {
       sprintf(hi,"%s","TX");
       setBtn(BUTTON_TX,hi,_tx,_fickle);
       showBtn(BUTTON_TX);
    } else {
       sprintf(hi,"%s","RX");
       setBtn(BUTTON_TX,hi,_tx,_fickle);
...
void displayPDX::init() {
  ...
  setBtn(BUTTON_AGC,(char*)"FAST",false,false);
  setBtn(BUTTON_MODE,(char*)"FT8",false,false);
...
void displayPDX::demo() {

  setTX(true);
  delay(1000);

  setTriangle(0,true,false);
  delay(1000);
  setTriangle(0,false,false);
  setTriangle(1,true,false);
  delay(1000);
  setTriangle(1,false,false);
  
  for (uint32_t k=7074;k<7077;k++) {
     setFreq(k*1000);
     delay(1000);
  }
  ...
  setAGC(AGC_ATTACK_FAST);
  delay(1000);

  setAGC(AGC_ATTACK_SLOW);
  delay(1000);

  setAGC(AGC_GAIN_STEP);
  delay(1000);
...
void spectrumPDX::set(uint16_t f) {

    if (f>2400) {
       s[0].signal[BINS-1]=0x0f;
       return;
    }

    uint32_t pixel=f/5;
    uint32_t bin=pixel*1000;
    bin=bin/8;
    uint32_t x=pixel/8;
    x=x*1000;
    x=bin-x;
    x=x*8;
    uint16_t slot=x/1000;
    uint16_t v=0x00;
    v |= 1 << slot;
    bin=bin/1000;
...
void spectrumPDX::noise() {
    for (int i=0;i<20;i++) {
        uint16_t fsk=rand()%2399;
        if (randomSig()) {
           set(fsk);
        }
    }
}
void spectrumPDX::demo() {
...
void spectrumPDX::show() {


    fh=tft->fontHeight();
    for (int i=0;i<TIME;i++) {
    
    tft->drawFastHLine (b.xStart,b.yStart+(i*2)+1+25, b.width,TFT_BLUE);
  
    for (int j=0;j<BINS;j++) {
       int pixel=j*8;
       if ((pixel==0 || pixel==100 || pixel==200 || pixel==300 || pixel==400) && (i%2 ==0)) {
        
              tft->drawPixel(b.xStart+pixel, b.yStart+(i*2)+0+25, TFT_YELLOW); 
              tft->drawPixel(b.xStart+pixel, b.yStart+(i*2)+1+25, TFT_YELLOW); 
          ...
          for (int k=0;k<8;k++) {
              ...
              tft->drawPixel(b.xStart+pixel+k, b.yStart+(i*2)+1+25, c);  
...
void GUI_init() {
  ...
  d.addBtn(BUTTON_AGC,&btnAGC);
  d.addBtn(BUTTON_MODE,&btnMODE);
